...
const categoryLinks = computed(() => categories.value.map(item => ({
    ...
    link: `/catalog?filteredId=${item.id}`
})))
...
<template>
    <header class="max-w-screen-xl mx-auto px-5 mb-5 w-full">
        <div class="flex items-center relative z-20 justify-between py-4">
            <waterjet-burger :active="active" class="md:hidden relative" @toggle-menu="toggleMenu">
                <div :style="{transform: active ? 'translateX(0)' : 'translateX(-100%)'}" class="left-0 overflow-x-hidden overflow-y-auto px-5 transition transition-property-left duration-300 fixed bg-white top-24 bottom-0 gap-7 flex flex-col items-center w-full">
                    ...
                    <waterjet-nav-list class="mx-3.5 gap-x-6 lg:gap-x-12 w-full flex flex-col gap-5 items-start justify-between text-base lg:text-xl font-bold" withIcons :items="topLinks" />
                    <waterjet-nav-list class="flex flex-col gap-5 w-full items-start flex-wrap gap-x-14" :items="categoryLinks" />
                    <waterjet-location class="flex gap-2.5 w-full items-start text-base lg:text-xl font-bold" />
                </div>
            </waterjet-burger>
            <waterjet-nav-list class="hidden mx-3.5 gap-x-6 lg:gap-x-12 md:flex items-center justify-between text-base lg:text-xl font-bold" :items="topLinks" />
        ...
        <nav class="hidden md:block bg-gray-200 p-3.5 text-xl">
            <waterjet-nav-list class="flex items-center flex-wrap gap-x-14 hover:drop-shadow-sm" :items="categoryLinks" />
        </nav>
    </header>
</template>
